      ...
      */}
      <div className="flex flex-col h-full bg-gray-700 border border-gray-300 hover:border-[#7faeb2] transition-all duration-500 overflow-hidden relative">
        ...
        <div className="relative aspect-4/5 overflow-hidden">
          ...
          */}
          <div className="absolute top-4 right-4 z-20 bg-black/80 backdrop-blur-md px-3 py-1 border border-[#7faeb2]/30">
            <span className="text-[10px] text-[#7faeb2] uppercase tracking-widest font-cal font-bold">
              {product.categoria}
            </span>
          </div>
        </div>
        ...
        <div className="p-6 flex flex-col grow justify-between gap-4">
          <div>
            {/* MODIFICA: Sottolineatura colorata in hover (decoration-[#7faeb2]) 
            */}
            <h3 className="text-2xl text-white uppercase font-cal mb-2 group-hover:underline decoration-1 underline-offset-4 decoration-[#7faeb2]">
              {product.nome}
            </h3>
          ...
          <div className="flex items-center justify-between border-t border-white/10 pt-4 mt-auto">
            <div className="flex flex-wrap gap-2 max-w-[85%]">
               {product.target.slice(0, 3).map((t, i) => (
                   <span key={i} className="text-[9px] uppercase border border-white/20 px-2 py-1 text-white/40 rounded-full">
                       {t}
                   </span>
               ))}
            </div>
            
            {/* MODIFICA: Freccia che diventa colorata in hover (group-hover:text-[#7faeb2]) 
            */}
            <span className="text-white group-hover:text-[#7faeb2] group-hover:translate-x-2 transition-all duration-300">
                <FiArrowRight />
            </span>
          </div>
        </div>
